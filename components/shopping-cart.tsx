...
import { ShoppingBag, X, Plus, Minus, Trash2, Scale } from "lucide-react"
...
export default function ShoppingCart() {
    ...
    const { totalWeight, formattedWeight } = useMemo(() => {
        let weightInGrams = 0

        cartItems.forEach((item) => {
            if (item.weight && item.weightUnit) {
                let itemWeightInGrams = Number(item.weight)
        ...
        return { totalWeight: weightInGrams, formattedWeight }
    }, [cartItems])
